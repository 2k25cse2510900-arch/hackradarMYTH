...
const listAlerts = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, { alerts }, "Alerts fetched"));
});
...
const updateAlert = asyncHandler(async (req, res) => {
  const alert = await alertService.updateAlert(req.user._id, req.params.id, req.body);
...
const deleteAlert = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, null, "Alert deleted"));
});
